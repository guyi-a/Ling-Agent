...
import { ArrowLeft, ChevronLeft, ChevronRight, CheckCircle, Clock, AlertTriangle, Play, Sun, Moon } from 'lucide-react'
...
const TAG_COLORS: Record<string, string> = {
  ...
  '综合': 'text-indigo-700 bg-indigo-100 dark:text-indigo-300 dark:bg-indigo-900/40',
  '自我效能': 'text-emerald-700 bg-emerald-100 dark:text-emerald-300 dark:bg-emerald-900/40',
  '孤独': 'text-slate-700 bg-slate-100 dark:text-slate-300 dark:bg-slate-900/40',
}
                    ...
                    <div key={group.key}>
                      <div className="flex items-center gap-3 mb-5">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-gray-400 dark:text-gray-500">{group.subtitle}</p>
                        </div>
                      </div>
                        ...
                        {groupScales.map(s => {
                          ...
                          return (
                            ...
                            >
                              ...
                              <p className="text-xs text-gray-500 dark:text-gray-400 mb-3 flex-1">{s.description}</p>
                              {latest && (
                                <div className="flex items-center gap-2 mb-3 text-xs flex-wrap">
                                  <span className="text-gray-400 dark:text-gray-500">上次:</span>
                                  {(!latest.result_type || latest.result_type === 'severity') ? (
                                    <>
                                      <span className="font-medium text-gray-700 dark:text-gray-300">{latest.total_score}分</span>
                              ...
                              <div className="flex items-center justify-between">
                                ...
                                <div className="flex items-center gap-2">
                                  ...
                                  <button
                                    ...
                                    className="flex items-center gap-1.5 px-4 py-1.5 rounded-lg text-sm font-medium bg-gray-900 dark:bg-white hover:bg-gray-800 dark:hover:bg-gray-100 text-white dark:text-gray-900 transition-colors"
                                  >
          ...
          <div className="mt-4">
                  ...
                  <button
                    ...
                    className={`w-full text-left px-5 py-3.5 rounded-xl border text-sm transition-all ${
                      answers[question.id] === opt.score
                        ? 'border-gray-900 dark:border-white bg-gray-900/5 dark:bg-white/5 text-gray-900 dark:text-gray-100 font-medium'
                        : 'border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-white/[0.03] hover:border-gray-300 dark:hover:border-gray-600'
                    }`}
                  >
                    {opt.label}
                  </button>
            ...
            <div className="flex justify-between mt-6">
              <button
                ...
                disabled={currentQ === 0}
                className="flex items-center gap-1 px-4 py-2 text-sm text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-white/5 rounded-lg disabled:opacity-30 transition-colors"
              >
              ...
              {currentQ < activeQuestions.length - 1 ? (
                <button
                  ...
                  className="flex items-center gap-1 px-4 py-2 text-sm bg-gray-900 dark:bg-white hover:bg-gray-800 dark:hover:bg-gray-100 text-white dark:text-gray-900 rounded-lg disabled:opacity-30 transition-colors"
                >
                ...
                <button
                  ...
                  className="flex items-center gap-1 px-5 py-2 text-sm bg-gray-900 dark:bg-white hover:bg-gray-800 dark:hover:bg-gray-100 text-white dark:text-gray-900 rounded-lg disabled:opacity-30 transition-colors"
                >
          ...
          <div className="mt-4">
            <div className="bg-white dark:bg-white/[0.05] rounded-2xl border border-gray-100 dark:border-gray-800/80 p-8 text-center">
              ...
              {resultType === 'severity' && (
                <>
                  <div className="mb-5">
                    <div className="text-5xl font-bold text-gray-900 dark:text-gray-100 mb-1">{result.total_score}</div>
                    <div className="text-sm text-gray-400 dark:text-gray-500">总分</div>
                  </div>
              ...
              {resultType === 'personality' && resultDetail && (
                <>
                  ...
                  <div className="text-4xl font-bold text-gray-900 dark:text-gray-100 mb-1 tracking-widest">{resultDetail.type}</div>
                  <div className="text-base font-medium text-teal-700 dark:text-teal-300 mb-3">{resultDetail.title}</div>
                  <p className="text-sm text-gray-500 dark:text-gray-400 mb-6 max-w-md mx-auto">{resultDetail.description}</p>
                      ...
                      {Object.entries(resultDetail.dimensions as Record<string, Record<string, number>>).map(([dim, counts]) => {
                        ...
                        return (
                          <div key={dim} className="flex items-center gap-2 text-xs">
                            <span className={`w-5 text-right font-bold ${lv >= rv ? 'text-teal-700 dark:text-teal-300' : 'text-gray-400 dark:text-gray-500'}`}>{left}</span>
                            <div className="flex-1 h-3.5 bg-gray-200 dark:bg-gray-800 rounded-full overflow-hidden relative">
                              <div className="absolute inset-0 flex">
                                <div className="h-full bg-teal-400 dark:bg-teal-500 rounded-l-full transition-all" style={{ width: `${leftPct}%` }} />
                              </div>
                            </div>
                            <span className={`w-5 font-bold ${rv > lv ? 'text-teal-700 dark:text-teal-300' : 'text-gray-400 dark:text-gray-500'}`}>{right}</span>
                          </div>
                        )
                      })}
                    </div>
              ...
              {resultType === 'label' && resultDetail && (
                <>
                  ...
                  <div className="text-3xl font-bold text-gray-900 dark:text-gray-100 mb-2">{resultDetail.title}</div>
                  {resultDetail.subtitle && (
                    <div className="text-sm text-gray-500 dark:text-gray-400 mb-3">{resultDetail.subtitle}</div>
                  )}
                  ...
                  {resultDetail.pattern && (
                    <div className="inline-block px-4 py-1.5 rounded-lg bg-gray-100 dark:bg-gray-800 text-xs font-mono text-gray-500 dark:text-gray-400 mb-4 tracking-wider">
                      {resultDetail.pattern}
                    </div>
                  )}
                  {resultDetail.dimensions && (
              ...
              <button
                ...
                className="px-8 py-2.5 bg-gray-900 dark:bg-white hover:bg-gray-800 dark:hover:bg-gray-100 text-white dark:text-gray-900 rounded-xl text-sm font-medium transition-colors"
              >
                返回
              </button>
            </div>
